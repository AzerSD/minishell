...
void save_matches(struct s_word **head, struct s_word **tail, glob_t *glob, char **matches);
void link_to_existing_list(struct s_word **words, struct s_word **pw, struct s_word **head, struct s_word **tail, struct s_word **w);
 ...
 *
 * This function loops over the matched filenames (`matches`), skipping entries that are `.` or `..` or start with `./`.
 * It creates a new linked list of words (`head`) from these matches, appending new matches to the end (`tail`) of the list.
*/
void save_matches(struct s_word **head, struct s_word **tail, glob_t *glob, char **matches) {
 ...
 * If the word being expanded is the first word in the list, the head of the list is changed.
 * Otherwise, the previous word's `next` pointer is updated to point to the new list.
 * Finally, the word that was expanded is freed, and `w` is updated to point to the last word that was added (`tail`).
*/
void link_to_existing_list(struct s_word **words, struct s_word **pw, struct s_word **head, struct s_word **tail, struct s_word **w) {
    if (*w == *words)
        *words = *head;
