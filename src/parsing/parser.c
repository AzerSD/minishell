...
/*   Updated: 2023/07/29 22:49:16 by lhasmi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_node_type get_node_type(t_token_type state)
{
    if (state == PARSE_DEFAULT)
        ...
        return NODE_HEREDOC;
    else if (state == TOKEN_OR)
        return NODE_OR;
    else if (state == TOKEN_AND)
        return NODE_AND;
    else if (state == TOKEN_LEFT_BRACKET)
        return NODE_LEFT_BRACKET;
    else if (state == TOKEN_RIGHT_BRACKET)
        return NODE_RIGHT_BRACKET;
    else
        return NODE_VAR;
}
...
t_node *parse_cmd(t_token *tok, t_curr_tok *curr) {
    ...
    do {
        ...
        if (type == NODE_PIPE) {
            ...
            // handle the OR operator here, similar to how you handle PIPE
            ptr = p_or(&ptr, &parent, &first_pipe);
        } else if (type == NODE_AND) {
            ptr = p_and(&ptr, &parent, &first_pipe);
        } else if (type == NODE_LEFT_BRACKET || type == NODE_RIGHT_BRACKET) {
            ptr = p_brackets(&ptr, &parent, &first_pipe);
